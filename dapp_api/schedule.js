...
schedule.scheduleJob('0/5 * * * * *', async function(){
  ...
  try {
    ...
    let query = `select * from t_expect where game_id = ${game_id} and ${current} > end_timestamp order by num desc limit 1`;
    ...
    query = `select count(*) as record from t_open_result where open_timestamp > ${open_timestamp} and game_id = ${game_id} and num = '${expect.num}' limit 1`;
    open_result = await sqlite.get(query);
    ...
    //Step 3: 转帐开奖
    //Step 3.1 根据当前环境获取配置
    let env = await common.currentEnv();
    let account = config[env];
    if(account == undefined || account.privateKey == null) {
      return;
    }
...
schedule.scheduleJob('0/3 * * * * *', async function(){
  ...
  console.log('************** 查询块高度   -    end...  '+dayjs().format("YYYY-MM-DD HH:mm:ss"));
});



// schedule.scheduleJob('0/3 * * * * *', function(){
//   console.log('3 - The answer to life, the universe, and everything!'+new Date());
// });

// schedule.scheduleJob('0/5 * * * * *', function(){
//   console.log('5 - The answer to life, the universe, and everything!'+new Date());
// });
