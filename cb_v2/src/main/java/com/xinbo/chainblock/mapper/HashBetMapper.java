...
public interface HashBetMapper extends BaseMapper<HashBetEntity> {
    ...
    @Update("update t_hash_bet set transaction_id=#{entity.transactionId},block_hash=#{entity.blockHash},block_height=#{entity.blockHeight},network=#{entity.network},profit_money=#{entity.profitMoney},payout_money=#{entity.payoutMoney},update_time=#{entity.updateTime},result=#{entity.result},status=#{entity.status} where id = #{entity.id}")
